...
<Collapsible.Root
	class="relative flex dark:text-neutral-200 text-gray-800 {rootClasses}"
>
	<div class="flex items-center rounded-lg {labelClasses}">
		...
		>
			...
			{/if}
			{#if hasIcon}
				<FlowbiteChevronSortOutline
					class="size-4 {iconClasses}"
				/>
			{/if}
		</Collapsible.Trigger>
	</div>
	...
	<Collapsible.Content
		class="absolute top-full z-10 mt-2 bg-white dark:bg-gray-800 border-1 border-gray-400 dark:border-gray-300 {popoverClasses}"
	>
		{#if popoverContent}
